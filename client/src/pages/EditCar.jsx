...
const EditCar = ({ title }) => {
    ...
    // Fetch Builder Data & Existing Car Config
    useEffect(() => {
        const fetchData = async () => {
            try {
                const optRes = await CarsAPI.getAllOptions()
                const incRes = await CarsAPI.getIncompatibilities()
                const carRes = await CarsAPI.getCarById(id)

                if (optRes.success && incRes.success && carRes.success) {
                    ...
                    // Match current selections with option objects
                    setSelectedExterior(opts.find(o => o.id === carData.exterior_id))
                    setSelectedRoof(opts.find(o => o.id === carData.roof_id))
                    setSelectedWheels(opts.find(o => o.id === carData.wheels_id))
                    setSelectedInterior(opts.find(o => o.id === carData.interior_id))
                }
            } catch (err) {
                console.error('Error loading edit builder data:', err)
            } finally {
                setLoading(false)
            }
        }
    ...
    const handleUpdateCar = async (e) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                alert(`Error updating car: ${response.error}`)
            }
        } catch (err) {
            console.error(err)
    ...
    return (
        <main className="container">
            <form onSubmit={handleUpdateCar} style={{ width: '100%' }}>
